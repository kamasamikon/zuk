 ...
 */
typedef enum {
	KMCRC_STARTEVT_SUCCESS = 0,
	/**< When  UI call setRecState(KMCRS_START),the bottom will often sent the result */
	KMCRC_STARTEVT_NOSPACE,	/**< When  no space  for record */
	KMCRC_STARTEVT_DIRLOST,	/**< The dir of record do not exist */
	KMCRC_STARTEVT_DEVICELOST
		/**< Broadcast device is removed */
} KMCRC_STARTEVT;
 ...
 */
typedef enum {
	KMCRC_ENDEVT_SUCCESS = 0,
	/**< When  UI call setRecState(KMCRS_END),the bottom will often sent the result */
	KMCRC_ENDEVT_NOSPACE,	/**< When  no space  for record,the bottom will sent the result and auto stop to record */
	KMCRC_ENDEVT_SIGNALBREAK,
	/**< When signal is  breaked,the bottom will sent the result and auto stop to record */
