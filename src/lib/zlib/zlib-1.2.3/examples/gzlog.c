...
#include "zlib.h"               /* zei_deflateInit2(), zei_deflate(), zei_deflateEnd() */
...
local char empty_gz[] = {
    ...
    8,                          /* compression method is zei_deflate */
...
int gzlog_write(void *obj, char *data, size_t len)
{
    ...
    do {
        ...
        if (write(log->fd, data, some) != some)
            return 1;
        log->crc = zei_crc32(log->crc, data, some);
        log->len += some;
        len -= some;
        data += some;
        log->stored += some;
...
/* recompress the remaining stored zei_deflate data in place */
local int recomp(gz_log *log)
{
    ...
    strm.zfree = Z_NULL;
    strm.opaque = Z_NULL;
    if (zei_deflateInit2(&strm, Z_BEST_COMPRESSION, Z_DEFLATED, -15, 8,
    ...
    while (len >= 5) {
        ...
        if (strm.avail_in != 0) {
            if (len < strm.avail_in)
                break;
            len -= strm.avail_in;
            (void)zei_deflate(&strm, Z_NO_FLUSH);
            if (strm.avail_in != 0 || strm.avail_out == 0)
                break;
        }
    }
    (void)zei_deflate(&strm, Z_SYNC_FLUSH);
...
int gzlog_close(void *obj)
{
    ...
    /* if more than 1 MB stored, then time to zei_compress it */
