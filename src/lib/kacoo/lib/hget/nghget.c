		...
		kvoid *a_ur0, kvoid *a_ur1, kvoid *a_ur2, kvoid *a_ur3, kint a_rsn)
{
	...
	while (ent != &__g_req_hdr) {
		...
		if (!r->tsk && !r->can && !kstr_icmp(r->host, s->host, 0)
				&& r->port == s->port) {
			found = ktrue;
			break;
		}
...
static kint connect_and_receive(hg_skt *s, hg_req *r)
{
	...
	if (-1 == s->socket || r->port != s->port || strcmp(r->host, s->host)) {
		...
		if (PGEC_SUCCESS != (ret = hget_connect(PROT_HTTP, "", "", r->host, r->path, r->port, &s->socket)))
			return PGEC_CONNECT;
	}
	...
	ret = hget_recv(s->socket, r->host, r->path, "", kfalse, knil, &datbuf, &datlen, &hdrbuf, &hdrlen);
...
static kvoid reset_skt_timer(hg_skt *s)
{
	s->timer = ktmr_set(knil, 5000, kfalse, om_skt_tmr, (kvoid *) s, (kvoid *) s->ref, knil, knil);
}
 ...
 */
kint Http_GetWebObject(kvoid *ses, kchar *pObjectUrl, kvoid *WebO_cbk,
		kchar *sSavePathName, kuint uiTimeout,
...
static kint Http_WebObjectCallback(kbean a_hdl, kint a_error, kchar *a_datbuf, kint a_datlen)
{
	...
	if (!strstr(r->pObjectUrl, "?")) {
		...
		tranobj_callback(ses, 0,	/* iHandle: unknown */
				error,	/* iStatus */
				objid,	/* uiObjectID */
				knil,	/* pObjectName */
