...
#if (defined(__WIN32__) || defined(__WINCE__))
#define CAN_CONTI() ((WSAETIMEDOUT == socket_last_error()) || (WSAEINTR == socket_last_error()))
#else
        ...
        kchar **a_datbuf, kint *a_datlen, kchar **a_hdrbuf, kint *a_hdrlen)
{
    ...
    do {
        ...
        if (0 == ret) {
            kerror(("hget_recv: recv failed: socket closed.\n"));
        }
        if (ret >= DATA_LEN) {
            cur_len += ret;
        }
    ...
    if (ret > 0) {
        ...
        if (!pTemp) {
            kerror(("hget_recv: Can not get Response code\n"));
            rc = PGEC_HEADER;
            goto done;
        }
        resp_code = atoi(pTemp + 9);
        if (resp_code < 200 || resp_code >= 300) {
            kerror(("hget_recv: resp_code (%d) tell an error\n", resp_code));
            rc = PGEC_RESP_CODE;
            goto done;
        }
        ...
        if (!pTemp) {
            kerror(("hget_recv: Can not get content length\n"));
            rc = PGEC_CONTENT_LEN;
            goto done;
        }
        ...
        pNext = *a_datbuf;
        if (!pNext) {
            kerror(("hget_recv: Can not alloc memory\n"));
            rc = PGEC_MEMORY;
            goto done;
        }
