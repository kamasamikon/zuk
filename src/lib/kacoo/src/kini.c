...
static kbool parse_file(const kchar *a_sec, const kchar *a_key,
		const kchar *a_buf, kint a_buflen,
		kint *a_sec_s, kint *a_sec_e, kint *a_key_s, kint *a_key_e, kint *a_val_s, kint *a_val_e)
{
	const kchar *p = a_buf;
	...
	*a_sec_e = *a_sec_s = *a_key_e = *a_key_s = *a_val_s = *a_val_e = -1;
	...
	while (!isend(a_buflen, i)) {
		...
		if ((0 == i || isnewline(p[i - 1]))
			...
			do {
				i++;
			} while (!isrightbrace(p[i])
					&& !isend(a_buflen, i));

			if (!isend(a_buflen, i)
					&& (seclen == (i - section_start))
					&& (0 == strncmp(p + section_start, a_sec, i - section_start))) {
