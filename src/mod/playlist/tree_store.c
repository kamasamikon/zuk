    ...
    while (month->label)
    {
        ...
        while (holiday->label)
        {
            ...
            gchar *title = g_strdup_printf("<span color='%s'>%s</span>\n<span color='%s' size='smaller'>%s%s%s</span>",
                    "red", month->label, "blue", " shit ", holiday->alex ? "true" : "false", " oyes");
...
static void add_columns (GtkTreeView *treeview)
{
    ...
    gtk_tree_view_column_pack_start(column, rend, FALSE);
    gtk_tree_view_column_set_attributes(column, rend, "pixbuf", RATE_ICON_COLUMN,
            "visible", RATE_ICON_VISIBLE_COLUMN,
            NULL);
}
...
GtkWidget * do_tree_store (GtkWidget *do_widget)
{
    if (!window)
    {
        ...
        g_signal_connect( G_OBJECT( grep_button ), "toggled", G_CALLBACK( cb_search_toggled ), GTK_ENTRY( entry ) );
